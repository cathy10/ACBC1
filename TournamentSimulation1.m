...
surveyfile='Tournament1.xlsx'; % Name of Survey Data File
totalruns=10;  
...
Survey1Attr(i,1).numberLevels=3; %%Specify the attribute levels
end
...
% most FBOs prefer more external fundings
 PopAttr(1,1).Mean=100;PopAttr(1,2).Mean=50;PopAttr(1,3).Mean=-150;
 PopAttr(1,1).Sd=10;PopAttr(1,2).Sd=10;PopAttr(1,3).Sd=10;
% most do smallest scale response, some do largest scale 
 PopAttr(2,1).Mean=40;PopAttr(2,2).Mean=-120;PopAttr(2,3).Mean=90;
 PopAttr(2,1).Sd=12;PopAttr(2,2).Sd=10;PopAttr(2,3).Sd=10;
% most responde when most vulnerable are in need, some responde when most vulnerable are unknown 
 PopAttr(3,1).Mean=90;PopAttr(3,2).Mean=-130;PopAttr(3,3).Mean=40;
 PopAttr(3,1).Sd=8;PopAttr(3,2).Sd=12;PopAttr(3,3).Sd=12;
% most FBOs have local parterns, almost none work with outide organization
% that they haven't worked with before
 PopAttr(4,1).Mean=140;PopAttr(4,2).Mean=60;PopAttr(4,3).Mean=-200;
 PopAttr(4,1).Sd=5;PopAttr(4,2).Sd=15;PopAttr(4,3).Sd=5;
